...
I have this condition that loads specific data based on selected primary goal. Currently our code just set the product whatever comes from the api call. I wanted to change is on fallback, if the API is not available, we should be loading below mock data. Can you set this up. You can just create a constant for this each mock data if it is the easy way to setup

If  "balance", Return below JSON:
...
If  primaryGoal is "maximize", Return below JSON:
{
  "asOfDate": "2026-02-15",
...
if primaryGoal is "reduce", return below json:
